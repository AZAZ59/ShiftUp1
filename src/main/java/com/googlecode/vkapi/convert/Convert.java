...
final class Convert {
    ...
    public static VkWallMessage toVkWallMessage(JsonNode node) {
        ...
        builder.addCommentsCount(node.get("comments").get("count").getIntValue());
        builder.addLikesCount(node.get("likes").get("count").getIntValue());
        builder.addRepostCount(node.get("reposts").get("count").getIntValue());
    ...
    public static VkGroup toVkGroup(JsonNode node) {
        if(node.isInt())return null;
        int gid = node.get("gid")==null?node.getIntValue():node.get("gid").getValueAsInt();
        ...
        if (members_count != null) {
            builder.addMembersCount(members_count.getValueAsInt());
        }
